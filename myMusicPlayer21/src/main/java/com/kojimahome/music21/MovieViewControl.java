...
import com.kojimahome.music21.VideoView;
...
//    private MediaController mMediaController;
    ...
    public MovieViewControl(View rootView, Context context, Uri videoUri) {
        ...
        mMediaController = new MyMediaController(context);
//        mMediaController = new MediaController(context);
    ...
    public void onAbPointsSelected(Intent intent) {
    	...
    	mMediaController.onAbPointsSelected(intent);
    	}
    }
    
    public void onSpeedSelected(int speed) {
    	if (mMediaController != null) {
        	mMediaController.onSpeedSelected(speed);
        	}
    }
    
    public int getSpeedIdx() {
    	if (mMediaController != null) {
        	return mMediaController.getSpeedIdx();
        	} else {
        		return 1;
        	}
    }
    ...
    public void onResume() {
        if (mPositionWhenPaused >= 0) {
         // Only when debugged
            if (mMediaController != null) {
            	if (mMediaController.ifAbSelectedFromList() != true) {
                    mVideoView.setVideoURI(mUri);
    ...
    public void onConfigurationChanged () {
    	mMediaController.refreshController();
    }

...
public class Mp3File extends FileWrapper {
	...
	private void init() throws IOException, UnsupportedTagException, InvalidDataException {
		...
		try {
//			Log.i("Mp3File","before initId3v1Tag called: ");
			initId3v1Tag(file);
//			Log.i("Mp3File","before scanFile called: ");
			scanFile(file);
			if (startOffset < 0) {
				throw new InvalidDataException("No mpegs frames found");
			}
//			Log.i("Mp3File","before initId3v2Tag called: ");
			initId3v2Tag(file);
			if (scanFile) {
//				Log.i("Mp3File","before initCustomTag called: ");
				initCustomTag(file);
			}
		} finally {
			file.close();
...
//		Log.i("Mp3File","before preScanFile called: ");
		int fileOffset = preScanFile(file);
		file.seek(fileOffset);
		boolean lastBlock = false;
		int lastOffset = fileOffset;
//		Log.i("Mp3File","before while loop: ");
		while (!lastBlock) {

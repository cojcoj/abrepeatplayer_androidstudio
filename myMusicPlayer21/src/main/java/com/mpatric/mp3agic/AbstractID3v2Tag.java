...
public abstract class AbstractID3v2Tag implements ID3v2 {
	...
	public static final String ID_UNSYNCHRONIZED_LYRICS = "USLT";
	...
	public String getUnsynchLyrics() {
		ID3v2LyricFrameData frameData;
//		if (obseleteFormat) frameData = extractTextFrameData(ID_UNSYNCHRONIZED_LYRICS);
//		else frameData = extractTextFrameData(ID_UNSYNCHRONIZED_LYRICS);
		frameData = extractLyricFrameData(ID_UNSYNCHRONIZED_LYRICS);
		if (frameData != null && frameData.getLyric() != null) return frameData.getLyric().toString();
		return null;
	}
	...
	private ID3v2LyricFrameData extractLyricFrameData(String id) {
		ID3v2FrameSet frameSet = frameSets.get(id);
		if (frameSet != null) {
			ID3v2Frame frame = (ID3v2Frame) frameSet.getFrames().get(0);
			ID3v2LyricFrameData frameData;
			try {
				frameData = new ID3v2LyricFrameData(useFrameUnsynchronisation(), frame.getData());
				return frameData;
			} catch (InvalidDataException e) {
				// do nothing
			}

...
//	public static final String IS_RUNNING = "com.kojimahome.music21.standoutwindow.isrunning";
	...
	public int mHiddenNotificationId = 0;
	...
	public static void show(Context context,
		...
		isRunning = true;
//		Log.i(TAG, "in show isRunning set to true");
	...
	public void onCreate() {
		...
		isRunning = true;
//		Log.i(TAG, "onCreate isRunning set to true");
	}
...
//		Log.i(TAG, "onDestroy called");
		isRunning = false;
//		MusicUtils.setBooleanPref(mContext,IS_RUNNING, false);
	}
	...
	public PopupWindow getDropDown(final int id) {
		...
		items.add(new DropDownListItem(
				android.R.drawable.ic_menu_close_clear_cancel, getResources().getString(R.string.quit)
						, new Runnable() {
					...
					public void run() {
						closeAll();
						isRunning = false;
//						Log.i(TAG, "isRunning set to false in DropDownListItems");
					}
				}));
...
//		isRunning = false;
	...
	public final synchronized void hide(int id) {
		...
		if (Utils.isSet(window.flags, StandOutFlags.FLAG_WINDOW_HIDE_ENABLE)) {
			...
			mHiddenNotificationId = getClass().hashCode() + id;
			mNotificationManager.notify(mHiddenNotificationId,
					notification);
//			Log.i(TAG,"Notify ID:"+(getClass().hashCode() + id));
